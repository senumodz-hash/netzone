...
def require_api_keys(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if public_key != PUBLIC_KEY or secret_key != stored_secret:
            return jsonify({'error': 'Invalid API keys'}), 403
...
def api_v2rays():
    try:
        ...
        return jsonify({'success': True, 'count': len(v2rays.get('vpn_configs', [])), 'data': v2rays}), 200
...
def api_apps():
    try:
        ...
        return jsonify({'success': True, 'count': len(apps.get('apps', [])), 'data': apps}), 200
    except Exception as e:
        return jsonify({'error': 'Internal server error', 'message': str(e)}), 500

@app.route('/whatsapp')
def whatsapp():
    return redirect("https://chat.whatsapp.com/Hre9DcY71UvC32oMVwwUrE")

@app.route('/discord')
def discord():
    return redirect("https://discord.gg/DhPZ8uMv4v")
